...
### Prompt 1.1: Widget Foundation
```
Create the core foundation for an embeddable webchat widget with the following specifications:
...
- Basic UI container that can be shown/hidden
...
### Prompt 2.1: Admin Panel Framework
```
Create an admin panel framework for configuring the webchat widget:
- Secure access mechanism (password protection or token-based)
...
### Prompt 2.2: Knowledge Base Management
```
Implement knowledge base management capabilities in the admin panel:
- File upload component with drag-and-drop and progress indicators
- URL scraping functionality that extracts content from web pages
- Text editor for direct knowledge input with formatting
- API configuration for external knowledge sources

Technical requirements:
- Handle multiple file formats (PDF, DOCX, TXT, CSV)
...
## Phase 4: Virtual Human Interface
...
### Prompt 4.3: Integration & Analytics
```
Complete the virtual human integration and add analytics capabilities:
- Seamless mode switching between chat widget and avatar
- Personalized welcome system based on user history
- Usage analytics dashboard for performance monitoring
