// eR assistant v1.4 - Bookmarklet Loader
...
(function() {
  ...
  // Inject HTML
  const html = `
    <button class="toggle-btn" id="toggle-btn">eR assistant</button>
      ...
      <div class="widget-content">
        ...
        <div class="section" id="health-section" style="display:none">
          ...
          <div class="section-content" id="content-health">
            ...
            <button class="run-analysis-btn" id="run-analysis-btn">Run Full Analysis</button>
          </div>
        </div>
  ...
  // Load saved width, clamped to valid range
  let savedWidth = parseInt(localStorage.getItem('er-widget-width')) || 500;
  let widgetWidth = Math.max(MIN_WIDGET_WIDTH, Math.min(MAX_WIDGET_WIDTH, savedWidth));
  ...
  // Apply saved width to widget
  const widget = document.getElementById('er-widget');
  if (widget) {
    widget.style.width = widgetWidth + 'px';
    widget.style.right = '-' + widgetWidth + 'px';
  }
  ...
  function initResize() {
    ...
    document.addEventListener('mousemove', (e) => {
      ...
      if (newWidth >= MIN_WIDGET_WIDTH && newWidth <= MAX_WIDGET_WIDTH) {
    ...
    document.addEventListener('mouseup', () => {
      if (isResizing) {
        ...
        localStorage.setItem('er-widget-width', widgetWidth);
      }
    });
  }
  ...
  function loadServiceFile(file) {
    ...
    // Security warning: Only upload JSON files you trust
    // This script runs in the page context and could execute malicious code
    
    const reader = new FileReader();
    reader.onload = (e) => {
      try {
        const serviceExport = JSON.parse(e.target.result);
        serviceData = serviceExport.service || serviceExport;
        
        // Validate that we have expected structure
        if (!serviceData || typeof serviceData !== 'object') {
          throw new Error('Invalid service structure - expected an object');
        }
        ...
        alert(`Failed to load service file:\n\n${error.message}\n\nPlease ensure you're uploading a valid BPA service export JSON file.`);
      }
    };
  ...
  function analyzeService(data) {
    ...
    // System variables to skip
    const systemVariables = new Set([
      'isFormValid', 'isCurrentPartATabValid', 'isUserLoggedIn', 
      'isPayedDigitally', 'isTotalCostZero', 'theProcedureIsFree', 
      'totalCost', 'is_submit_allowed'
    ]);
    
    function isSystemVariable(name) {
      if (!name) return false;
      if (systemVariables.has(name)) return true;
      if (name.includes('TabValid')) return true;
      if (name.startsWith('is') || name.startsWith('has') || name.startsWith('can')) return true;
      if (name.startsWith('rejection') || name.startsWith('sendbacktocorrections')) return true;
      if (name.startsWith('file') || name.startsWith('requirement') || 
          name.startsWith('cert') || name.startsWith('document')) return true;
      return false;
    }
    ...
    determinants.forEach(det => {
      if (det.determinantType === 'FORMFIELD' && det.targetFormFieldKey) {
        ...
        if (!isSystemVariable(det.targetFormFieldKey) && !allFieldKeys.has(det.targetFormFieldKey)) {
          errorsByCategory.determinant.push({
    ...
    // Check classifications for undefined determinants
    const determinantNames = new Set(determinants.map(d => d.name));
    classifications.forEach(cls => {
      if (cls.jsonClassificationDeterminants) {
        try {
          const clsDets = JSON.parse(cls.jsonClassificationDeterminants);
          if (Array.isArray(clsDets)) {
            clsDets.forEach(det => {
              if (det.determinantName && !isSystemVariable(det.determinantName)) {
                if (!determinantNames.has(det.determinantName)) {
                  errorsByCategory.determinant.push({
                    type: 'Broken Classification',
                    message: `Classification "${cls.name}" references undefined determinant "${det.determinantName}"`,
                    searchTerm: det.determinantName
                  });
                }
              }
              
              // Check if classification determinants have required values
              if (det.type === 'classification') {
                if (det.operator === 'IN' && !det.classificationValues) {
                  errorsByCategory.determinant.push({
                    type: 'Missing Values',
                    message: `Classification determinant in "${cls.name}" has operator IN but no classificationValues defined`,
                    searchTerm: det.determinantName || ''
                  });
                }
                if (det.subject === 'ALL' && !det.subjectValues) {
                  errorsByCategory.determinant.push({
                    type: 'Missing Values',
                    message: `Classification determinant in "${cls.name}" has subject ALL but no subjectValues defined`,
                    searchTerm: det.determinantName || ''
                  });
                }
              }
    ...
    determinants.forEach(det => {
      if (!usedDeterminantIds.has(det.oldId) && !isSystemVariable(det.name)) {
    ...
    let score = 100;
    score -= totalErrors * ERROR_PENALTY;
    score -= totalWarnings * WARNING_PENALTY;
  ...
  document.getElementById('toggle-btn').addEventListener('click', window.toggleWidget);
  document.getElementById('close-btn').addEventListener('click', window.toggleWidget);
  document.getElementById('run-analysis-btn').addEventListener('click', window.runAnalysis);
  ...
  console.log('✅ eR assistant v1.4 loaded');
